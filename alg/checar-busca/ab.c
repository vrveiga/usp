...
bool ab_checar_busca_rec(NO *raiz) {
    if (raiz->esq != NULL && raiz->dir != NULL) {
        return ab_checar_busca_rec(raiz->esq) + ab_checar_busca_rec(raiz->dir) +
                (item_get_chave(raiz->item) < item_get_chave(raiz->esq->item)) +
                (item_get_chave(raiz->item) > item_get_chave(raiz->dir->item));  
    } else if (raiz->esq != NULL) {
        return ab_checar_busca_rec(raiz->esq) +
                (item_get_chave(raiz->item) < item_get_chave(raiz->esq->item));
    } else if (raiz->dir != NULL) {
        return ab_checar_busca_rec(raiz->dir) +
                (item_get_chave(raiz->item) > item_get_chave(raiz->dir->item));  
    } else {
        return 0;
    }
...
int ab_checar_busca (AB *T) {
    ...
    return ab_checar_busca_rec(T->raiz);
}